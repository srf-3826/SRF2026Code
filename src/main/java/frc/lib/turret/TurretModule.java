...
public class TurretModule {
    public  int m_modNum;
    private TurretModuleConstants m_moduleConstants;
...
*/
    ...
    public TurretModule(int moduleNumber, TurretModuleConstants moduleConstants, CANBus swerveCanbus) {
        ...
        m_lastAngle = getAngle2d().getDegrees();

        setupModulePublishing();
    }

    public void setDesiredAngle(Rotation2d desiredAngle, boolean isOpenLoop) {
        ...
        // swerve module.
        // desiredAngle = TurretOptimize.optimize(desiredAngle, getAngle2d());
        double max = 90;
        double min = -90;
        if (desiredAngle.getDegrees()>max) {
            setAngle(max);
        } else if (desiredAngle.getDegrees()<min) {
            setAngle(min);
        } else {
            setAngle(desiredAngle.getDegrees());
        }
    }


    public void setAngle(double desiredAngle) {
        m_steerClosedLoop.Position = desiredAngle * TurretModuleConstants.TURRET_ROTATION_TO_DEGREES;
